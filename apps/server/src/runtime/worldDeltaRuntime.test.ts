...
describe("worldDeltaRuntime", () => {
  it("keeps bounded delta history and reports memory status", () => {
    ...
    replaceWorldDeltaHistory(history, [makeDelta(7)]);
    expect(history.map((delta) => delta.worldStateVersion)).toEqual([7]);
  ...
  it("returns replay deltas only when history is continuous through current version", () => {
    ...
    expect(getReplayDeltasFromVersion({ history, fromWorldStateVersion: 1, currentWorldStateVersion: 4 })).toEqual({
      ok: true,
      deltas: history,
    });
    expect(getReplayDeltasFromVersion({ history, fromWorldStateVersion: 4, currentWorldStateVersion: 4 })).toEqual({
      ok: true,
      deltas: [],
    });
    expect(getReplayDeltasFromVersion({ history: [makeDelta(2), makeDelta(4)], fromWorldStateVersion: 1, currentWorldStateVersion: 4 })).toEqual({
      ok: false,
    });
    expect(getReplayDeltasFromVersion({ history, fromWorldStateVersion: 1, currentWorldStateVersion: 5 })).toEqual({
      ok: false,
    });
  });
  ...
  it("captures and resets websocket delta size metrics", () => {
    ...
    const compactPayload = makeDelta(2, { c: { "country:a": { culture: 1, science: 0, religion: 0, colonization: 0, construction: 0, ducats: 0, gold: 0 } } });
...
function makeDelta(worldStateVersion: number, overrides?: Partial<WorldDelta>): WorldDelta {
  return {
    type: "WORLD_DELTA",
    turnId: 1,
    worldStateVersion,
