...
describe("createServerApp", () => {
  ...
  it("serves scenario-owned identity icons from the configured data root", async () => {
    const dataRoot = await makeTempDir();
    await mkdir(join(dataRoot, "scenarios", "demo", "assets", "identity"), { recursive: true });
    await writeFile(join(dataRoot, "scenarios", "demo", "assets", "identity", "riverine.png"), "identity-body");
    const app = createServerApp({ dataRoot });
    ...
    try {
      const address = server.address();
      if (!address || typeof address === "string") throw new Error("server did not bind to a port");
      const response = await fetch(`http://127.0.0.1:${address.port}/scenario-assets/demo/assets/identity/riverine.png`);
