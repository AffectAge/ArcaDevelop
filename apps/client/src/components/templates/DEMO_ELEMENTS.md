...
Use these exact component names when asking an AI agent to build Arcanorum modals or reusable UI flows. Prefer these components over one-off CSS inside a modal.
...
- All visible text must come from `apps/client/src/i18n/uiText.ts` with EN/RU values.
- Use the exported components from `apps/client/src/components/templates/index.ts` where possible.
- Do not import from or recreate `apps/client/src/components/ui`; that folder is removed.
- When adding a reusable UI component, add it under `apps/client/src/components/templates`, export it from `index.ts`, and update this document in the same change.
