...
import { AlertTriangle, ArrowDownUp, BarChart3, LineChart, Settings2, ShieldBan, TrendingDown } from "lucide-react";
...
export function MarketModal({ open, onClose, token, countryId, countryName, mode = "both", title = "Рынок" }: Props) {
  ...
  const marketNameById = useMemo(
    () => Object.fromEntries(marketsCatalog.map((row) => [row.id, row.name] as const)),
    [marketsCatalog],
  );
